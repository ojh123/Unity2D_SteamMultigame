...
## 6. 학습 및 성과
+ **멀티플레이 네트워크** 구조 설계 경험
  - Mirror와 Steamworks.NET을 연동하며, 서버-클라이언트 개념 및 네트워크 객체 동기화 흐름을 이해하고 적용했습니다.
 
+ **인터페이스** 기반 **설계 능력** 향상
  - 아이템 시스템을 IItemEffect 인터페이스 기반으로 설계하여, 새로운 아이템 효과를 손쉽게 추가 가능하게 확장성과 유지보수성을 확보했습니다.

+ **문제 해결 중심**의 **디버깅** 경험
  - Steam Lobby와 Mirror 세션 연동, UI 비동기 문제,플레이어 상태 동기화 등 멀티플레이 환경에서 발생하는 문제들을 실시간 디버깅과 구조 개선을 통해 해결했습니다.
